...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        public void StartClient()
        {
            ...
            natsConnection = cf.CreateConnection(opt);

            // Setup an event handler to process incoming messages.
            EventHandler<MsgHandlerEventArgs> handler = (sender, args) =>
            {
                GameObject player = Instantiate(playerPrefab, new Vector3(244, 8, 246), Quaternion.identity);
            };

            IAsyncSubscription subAsync = natsConnection.SubscribeAsync("blitz.playerConnected", handler);
            natsConnection.Publish("blitz.playerConnected", Encoding.UTF8.GetBytes("Game connected!"));
